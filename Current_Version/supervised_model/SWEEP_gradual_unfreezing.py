...
# Gradually unfreeze layers (starting from the last encoder block)
def unfreeze_layers(model, num_encoder_blocks, freeze_until_block):
    """
    Unfreeze layers progressively. The layers up to 'freeze_until_block' remain frozen.
    """
    for block_idx in range(num_encoder_blocks - 1, freeze_until_block - 1, -1):
        # Unfreeze the layers of the encoder block
        for param in model.segformer.encoder.block[block_idx].parameters():
...
sweep_config = {
    ...
    'parameters': {
        'palsa_weight': {'values': [2]},
        'lr': {'values': [1e-6,1e-7,1e-8]}
        }
}
...
def train():
    ...
    run = wandb.init(
        config={
            ...
            "freeze_encoder": 'gradual_unfreezing',  
    ...
    num_encoder_blocks = model.config.num_encoder_blocks
    freeze_until_block = num_encoder_blocks

    for epoch in range(epochs):
        if epoch % 20 == 0 and not epoch == 0:  # Unfreeze one block every 2 epochs
            # Save scheduler state before changing optimizer 
            scheduler_state = scheduler.state_dict()

            # change which parameters are optimized (unfreeze)
            freeze_until_block -= 1
            unfreeze_layers(model, num_encoder_blocks, freeze_until_block)
            for param_group in optimizer.param_groups:
                param_group['params'] = filter(lambda p: p.requires_grad, model.parameters())

            # Re-instantiate the scheduler with the updated optimizer
            scheduler = get_cosine_schedule_with_warmup(
                optimizer,
                num_warmup_steps=warmup_steps,
                num_training_steps=total_steps
            ...
            # Load the saved state back into the scheduler
